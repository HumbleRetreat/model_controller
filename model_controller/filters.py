...
def create_filter_model(model: Type[DeclarativeMeta]) -> Type[FiltersBase]:
    ...
    for column in model.__table__.columns:
        ...
        if isinstance(column.type, Integer):
            ...
            model_fields[column.name] = (Optional[int], Field(None, description=f"Filter by {column.name}"))
            model_fields[f"{column.name}_lt"] = (Optional[int], Field(None, description=f"Less than {column.name}"))
            model_fields[f"{column.name}_gt"] = (Optional[int], Field(None, description=f"Greater than {column.name}"))

        elif isinstance(column.type, tuple(string_classes)):
            # For string fields (e.g., name), support exact match or partial match
            model_fields[column.name] = (Optional[str], Field(None, description=f"Filter by {column.name}"))
            model_fields[f"{column.name}_like"] = (
                Optional[str], Field(None, description=f"Partial match for {column.name}"))
